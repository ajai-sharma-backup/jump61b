...
abstract class Board extends Observable {
    ...
    /** Clears the board and retains its current size. */
    void clear() {
        clear(size());
    }
    ...
    /** Returns the number of squares contained in the board. Size ^ 2. */
    int max() {
        return size() * size();
    }
    ...
    boolean isLegal(Side player, int n) {
        return (exists(n) && (get(n).getSide() != player.opposite()));
    }

    /** Returns true iff PLAYER is allowed to move at this point. */
    boolean isLegal(Side player) {
        for (int i = 1; i < max(); i += 1) {
            if (!isLegal(player, i)) { return false; }
        }
        return true;
    }
